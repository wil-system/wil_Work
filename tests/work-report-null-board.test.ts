...
test('work report form allows submitting without a department board', () => {
  ...
  assert.match(formSource, /name="department"/);
  assert.match(formSource, /departmentOptions\.map/);
  assert.equal(formSource.includes('보고서를 작성할 수 있는 부서 권한이 없습니다.'), false);
});

test('work report persistence stores departments separately and missing board ids as null', () => {
  const reportDbSource = read('lib/db/reports.ts');
  const actionSource = read('app/(workspace)/work-report/actions.ts');

  assert.match(actionSource, /department:\s*department\s*\|\|\s*undefined/);
  assert.match(reportDbSource, /department:\s*report\.department\s*\?\?\s*''/);
